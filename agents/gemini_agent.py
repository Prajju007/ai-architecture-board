"""Gemini agent: initial position, structured deliberation, final position.

Mirrors the GPT agent but uses the Google Gemini client. Deliberation
turns produce the same structured JSON format for consistency.
"""
...
from core.llm_client import get_gemini_client, call_with_retry
from core.token_tracker import extract_gemini_tokens, estimate_cost
...
def _record_call(state: dict, agent_name: str, prompt_tokens: int, completion_tokens: int) -> dict:
    ...
    cost = estimate_cost(GEMINI_MODEL, prompt_tokens, completion_tokens)
    return {
        "llm_calls": llm_calls,
...
def gemini_initial_position(state: dict) -> dict:
    """Generate Gemini's independent initial architecture proposal."""
...
"""
    ...
    response = call_with_retry(_call, agent_name="gemini_initial_position")
    content = response.text

    prompt_tokens, completion_tokens = extract_gemini_tokens(response)
    tracking = _record_call(state, "gemini_initial_position", prompt_tokens, completion_tokens)

    return {"gemini_initial_position": content, **tracking}


def gemini_deliberation(state: dict) -> dict:
    """One structured deliberation turn from Gemini.
    ...
    current_round = len([t for t in turns if t["author"] == "Gemini"]) + 1
...
{{
    ...
    "criticisms": [
        {{
            "target_claim": "the claim from GPT you are challenging",
...
"""
    ...
    response = call_with_retry(_call, agent_name=f"gemini_deliberation_r{current_round}")
    ...
    prompt_tokens, completion_tokens = extract_gemini_tokens(response)
    tracking = _record_call(state, f"gemini_deliberation_r{current_round}", prompt_tokens, completion_tokens)

    return {"deliberation_turns": turns, **tracking}


def gemini_final_position(state: dict) -> dict:
    """Generate Gemini's final position after all deliberation rounds."""
    client = get_gemini_client()
...
"""
    ...
    response = call_with_retry(_call, agent_name="gemini_final_position")
    content = response.text

    prompt_tokens, completion_tokens = extract_gemini_tokens(response)
    tracking = _record_call(state, "gemini_final_position", prompt_tokens, completion_tokens)

    return {"gemini_final_position": content, **tracking}